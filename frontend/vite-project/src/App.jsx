...
function App() {
      ...
      if (LocalSelecionado && dataHoraInicio && dataHoraFim) {
        ...
        const response = await fetch(`${API}/agendar`, {
          ...
          headers: {
            ...
            // Adicionar token no cabeçalho Authorization para autenticação para a rota protegida
  ...
  // Pegar token do localStorage e decodificar para obter nome do usuário
  const token = localStorage.getItem('token')
  const usuario = token ? JSON.parse(atob(token.split('.')[1])) : null
  const nomeUsuario = usuario ? usuario.nome : 'Usuário não identificado'

  // Verificar se o local selecionado já está agendado
  const localAgendado = agendamentos.some((agendamento) => agendamento.local === LocalSelecionado?.local)
                ...
                {locais.map((local) => (
                  <div key={local.id_local} className={`border-2 rounded-lg p-4 cursor-pointer ${LocalSelecionado?.id_local === local.id_local ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`} onClick={() => setLocalSelecionado(local)}>
                    <div className='flex justify-between'>
                      ...
                      {/* Caso o lugar esteja agendado em algum horário, ele aparecerá como indisponível */}
                      {local.local == agendamentos.map((agendamento) => {
                        return agendamento.local
                      }) ? <span className='bg-red-300 py-1 px-2 rounded-full'>Indisponível</span> : (
              ...
              {LocalSelecionado ? (
                <form onSubmit={handleAgendar}>
                  <div className='space-y-4'>
                    ...
                    {/* Botão de agendar, desabilitado caso o local já esteja agendado */}
                    {localAgendado ? (
                      <button type='submit' disabled className='w-full bg-red-600 py-2 px-4 rounded-lg text-white hover:bg-red-700 transition ease-in-out duration-300 cursor-not-allowed'>Local já agendado</button>)
                      : (
                ...
                <div className='space-y-6'>
                  {agendamentos.map((agendamento) => (
                    <div key={agendamento.id} className='border-2 border-gray-200 rounded-lg p-4'>
                      ...
                      <p className='font-semibold text-gray-600'>{nomeUsuario}</p>
